<?php
    ...
    if(isset($_GET["typeZoneConsult"])) // si une valeur en particulier est prête alors
    {
        ...
        $conn = new PDO($dsn);
            if ($conn) {
                $req = 'SELECT soin."dateS", soin."codeA", soin."typeS", soin."commentaireS", soin."nomZone", animal."especeA", soignant."specialite" FROM animal inner join soin on animal."codeA"=soin."codeA" inner join soignant on soin."refS"=soignant."refS" WHERE soin."codeA"=? AND soin."refS"=? AND soin."typeS"=? AND soin."nomZone"=?';
